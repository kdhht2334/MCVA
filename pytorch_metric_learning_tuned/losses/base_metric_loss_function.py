...
class BaseMetricLossFunction(torch.nn.Module):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple=None, ref_pos=None, ref_neg=None):
        """
        This has to be implemented and is what actually computes the loss.
        """
        raise NotImplementedError

    def forward(self, embeddings, labels, indices_tuple=None, ref_pos=None, ref_neg=None):
        """
        Args:
        ...
        loss = self.compute_loss(embeddings, labels, indices_tuple, ref_pos, ref_neg)
        if loss == 0:
