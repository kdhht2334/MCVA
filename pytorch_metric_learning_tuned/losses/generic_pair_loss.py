...
class GenericPairLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple, ref_pos, ref_neg):
        ...
        return self.loss_method(mat, labels, indices_tuple, ref_pos, ref_neg)

    def _compute_loss(self):
        raise NotImplementedError

    def mat_based_loss(self, mat, labels, indices_tuple, ref_pos, ref_neg):
        a1, p, a2, n = indices_tuple
        pos_mask, neg_mask = torch.zeros_like(mat), torch.zeros_like(mat)
        pos_mask[a1, p] = 1
        neg_mask[a2, n] = 1
        return self._compute_loss(mat, pos_mask, neg_mask, ref_pos, ref_neg)

    def pair_based_loss(self, mat, labels, indices_tuple, ref_pos, ref_neg):
        a1, p, a2, n = indices_tuple
        pos_pair, neg_pair = [], []
        if len(a1) > 0:
            pos_pair = mat[a1, p] * ref_pos
        if len(a2) > 0:
            neg_pair = mat[a2, n] * ref_neg
