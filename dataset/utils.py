...
def make_transform(is_train=True, is_inception=False, is_big=False, wh_dataset='cub'):
    
    resnet_sz_resize = 256
    resnet_sz_crop = 224 
    resnet_mean = [0.485, 0.456, 0.406]
    resnet_std = [1.229, 0.224, 0.225]
    ...
    if wh_dataset == 'cub':
        if not is_big:
            inception_transform = transforms.Compose(
               [
                RGBToBGR(),
                transforms.RandomResizedCrop(224) if is_train else Identity(),
                transforms.RandomHorizontalFlip() if is_train else Identity(),
            ...
            inception_transform = transforms.Compose(
               [
                RGBToBGR(),
                transforms.RandomResizedCrop(256) if is_train else Identity(),
                transforms.RandomHorizontalFlip() if is_train else Identity(),
                transforms.Resize(288) if not is_train else Identity(),
                transforms.CenterCrop(256) if not is_train else Identity(),
                transforms.ToTensor(),
                ScaleIntensities([0, 1], [0, 255]),
                transforms.Normalize(mean=inception_mean, std=inception_std)
               ])
        ...
        if not is_big:
            ...
            inception_transform = transforms.Compose(
               [
                ...
                transforms.RandomResizedCrop(256) if is_train else Identity(),
                transforms.RandomHorizontalFlip() if is_train else Identity(),
                transforms.Resize(256) if not is_train else Identity(),
                transforms.CenterCrop(256) if not is_train else Identity(),
                transforms.ToTensor(),
                ScaleIntensities([0, 1], [0, 255]),
                transforms.Normalize(mean=inception_mean, std=inception_std)
