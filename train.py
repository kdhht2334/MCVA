...
# Directory for Log
LOG_DIR = args.LOG_DIR + '/logs_{}/{}_{}_embedding{}_alpha{}_mrg{}_{}_lr{}_batch{}{}_for_vis'.format(args.dataset, args.model, args.loss, args.sz_embedding, args.alpha, 
                                                                                            args.mrg, args.optimizer, args.lr, args.sz_batch, args.remark)
# Wandb Initialization
wandb.init(project=args.dataset + '_PPGML', notes=LOG_DIR)
wandb.config.update(args)

os.chdir('<your/costum/path>')
data_root = os.getcwd()
...
elif args.loss == 'PPGML_PA':
    criterion = losses.PPGML_Proxy_Anchor(nb_classes = nb_classes, sz_embed = args.sz_embedding, mrg = args.mrg, alpha = args.alpha).cuda()
elif args.loss == 'Proxy_NCA':
    criterion = losses.Proxy_NCA().cuda()
elif args.loss == 'MS':
    criterion = losses.MultiSimilarityLoss().cuda()
elif args.loss == 'PPGML_MS':
    criterion = losses.PPGML_MultiSimilarityLoss().cuda()
elif args.loss == 'Contrastive':
    criterion = losses.ContrastiveLoss().cuda()
elif args.loss == 'PPGML_Cont':
    criterion = losses.PPGML_ContrastiveLoss().cuda()
elif args.loss == 'Triplet':
    criterion = losses.TripletLoss().cuda()
elif args.loss == 'PPGML_Tri':
    criterion = losses.PPGML_TripletLoss().cuda()
...
if args.loss == 'PPGML_Proxy_Anchor':
    param_groups.append({'params': criterion.proxies, 'lr':float(args.lr) * 200})
...
if args.optimizer == 'sgd': 
    ...
    opt = torch.optim.Adamax(param_groups, lr=float(args.lr), weight_decay = args.weight_decay)
    
scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=args.lr_decay_step, gamma = args.lr_decay_gamma)
#scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, mode="max", patience=4)
...
#        print("[INFO] x shape is {}".format(x.size()))
...
#    scheduler.step(losses_list[-1])
    ...
    if(epoch >= 0):
        ...
        if best_recall[0] < Recalls[0]:
            ...
            torch.save({'model_state_dict':model.state_dict()}, '{}/{}_{}_best_for_vis.pth'.format(LOG_DIR, args.dataset, args.model))
